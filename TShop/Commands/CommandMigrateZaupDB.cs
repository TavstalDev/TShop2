        ...
        public void Execute(IRocketPlayer caller, string[] args)
        {
            ...
            if (args.Length == 2)
            {
                try
                {
                    ...
                    UnturnedHelper.SendChatMessage(callerPlayer.SteamPlayer(), TShop.Instance.Translate(true, "success_migrate"));
                }
                catch (Exception ex)
                {
                    UnturnedHelper.SendChatMessage(callerPlayer.SteamPlayer(), TShop.Instance.Translate(true, "error_migrate_console"));
                    Logger.LogError("Migration error: " + ex);
                }
            }
            else
                UnturnedHelper.SendChatMessage(callerPlayer.SteamPlayer(), TShop.Instance.Translate(true, "error_command_migrate_args"));
        }
    }
}
