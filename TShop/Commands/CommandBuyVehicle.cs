    ...
    public class CommandBuyVehicle : IRocketCommand
    {
        ...
        public void Execute(IRocketPlayer caller, string[] args)
        {
            ...
            if (args.Length == 1)
            {
                ...
                if (cost == 0)
                {
                    UChatHelper.SendChatMessage(TShop.Instance,callerPlayer.SteamPlayer(),  "error_vehicle_buy_error");
                    return;
                }
                ...
                UChatHelper.SendChatMessage(TShop.Instance,callerPlayer.SteamPlayer(),  "success_vehicle_buy", asset.vehicleName, amount, cost, TShop.economyProvider.GetConfigValue<string>("MoneySymbol"));
            }
            else
                UChatHelper.SendChatMessage(TShop.Instance,callerPlayer.SteamPlayer(),  "error_command_buyvehicle_args");
        }
    }
}
